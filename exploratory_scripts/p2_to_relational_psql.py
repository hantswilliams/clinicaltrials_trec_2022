...
with open(f"temp/{files[0]}", "r") as f:
    ...
    if engine.execute(query).fetchall()[0][0]:
        ...
        for i in columnnames_noid:
            print(f'value {i}: ', json_content[i])
            query = f"INSERT INTO trials ({i}) VALUES ('{json_content[i]}' WHERE nct_id = '{json_content['nct_id'][0]}')"
            try:
                engine.execute(query)
            except:
                print('error: ', query)
